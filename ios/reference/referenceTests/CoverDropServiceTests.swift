...
final class EncryptedStorageTests: XCTestCase {
    ...
    let instance = EncryptedStorage(keyHandle: "test")

    /// Before any run, clean all data from storage
    override func setUpWithError() throws {
        try super.setUpWithError()
        try StorageManager.shared.deleteFile(file: instance.file)
    }
    ...
    /// Reads the current on-disk storage state for inspection by the individual tests. If the file is missing, `nil` is
    /// returned
    private func readTestStorageState() throws -> TestStorageState? {
        let fileURL = try StorageManager.shared.getFullUrl(file: instance.file)
