...
pub async fn minio_tunnel(aws_config: AwsConfig, port: u16) -> anyhow::Result<Child> {
    println!("Setting up tunnel to staging backup instance");
    ...
    let backup_instance = get_random_instance("coverdrop-k3s-backup", stage, &ec2_client).await?;
    let backup_instance_ip = backup_instance
        .private_ip_address
        .ok_or_else(|| anyhow::anyhow!("K3s instance private ip not found"))?;

    let child = create_tunnel(&backup_instance_ip, "9001", port, bastion_instance_id).await?;
