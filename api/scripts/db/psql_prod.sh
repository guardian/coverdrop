...
DB_ARN=$(aws resourcegroupstaggingapi get-resources \
  ...
  --tag-filters Key=Stage,Values=PROD               \
...
DB_SECRET_ARN=$(aws resourcegroupstaggingapi get-resources  \
  ...
  select (.Tags[].Key=="Stage" and .Tags[].Value=="PROD") |
...
SSH_COMMAND=$(ssm ssh --raw -t secure-collaboration,api,PROD --oldest --profile secure-collaboration)
