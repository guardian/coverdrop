...
impl ToJournalistPublishingService {
    pub fn new(key_state: KeyState, api_client: ApiClient, db: Database) -> Self {
        Self {
            key_state,
            api_client,
            db,
    ...
    pub async fn run(
        &self,
        mut inbound: mpsc::Receiver<UserToJournalistDeadDropContentWithCheckpointsAndMessageHashes>,
    ) -> anyhow::Result<()> {
        loop {
            ...
            loop {
                let post_dead_drop_attempt = self
                    .api_client
                    .post_user_to_journalist_dead_drop(&dead_drop)
                    .await;
            ...
            tracing::info!(
                "Saving U2J hashes and checkpoints: {:?}",
