...
class DeadDropParserTest {
    ...
    //
    // LEGACY (remove after the migration is complete, see #2998)
    //
    // verifyAndParseDeadDropsListOrThrow
    //

    @Test
    fun testVerifyAndParseDeadDropsList_whenGivenCorrectTestVector_thenVerifies_legacy() {
        val instance = DeadDropParser(libSodium, VerificationFailureBehaviour.THROW)

        val integrationTestVectors = IntegrationTestVectors(context, TestScenario.MinimalLegacy)
    ...
    fun testVerifyAndParseDeadDropsList_whenGivenWrongCoverNodeKey_andThrowBehaviour_thenThrows_legacy() {
        val instance = DeadDropParser(libSodium, VerificationFailureBehaviour.THROW)
    ...
    @Test
    fun testVerifyAndParseDeadDropsList_whenOneSignatureIsBad_andIgnoreBehaviour_thenAllOthersOk_legacy() {
        ...
        val alteredPublishedDeadDrops = publishedDeadDrops.deadDrops.map { deadDrop ->
            if (deadDrop.id == 2) {
                deadDrop.copy(cert = deadDrop.cert.reversed())
            } else {
                deadDrop
            }
        }
    ...
    @Test
    fun testVerifyAndParseDeadDropsList_whenOneCertIsBad_butCertificatesAreGood_thenAllOk() {
        val instance = DeadDropParser(libSodium, VerificationFailureBehaviour.DROP)
        ...
        val alteredPublishedDeadDrops = publishedDeadDrops.deadDrops.map { deadDrop ->
            if (deadDrop.id == 2) {
                // the wrong cert is not a problem, as the signature is valid
                deadDrop.copy(cert = deadDrop.cert.reversed())
            } else {
                deadDrop
            }
