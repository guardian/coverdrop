...
RUN cargo chef cook --recipe-path recipe.json --bin delivery-service
...
RUN cargo build --profile $CARGO_BUILD_PROFILE --bin delivery-service

# save the binary and then clean target folder
RUN mkdir /dist;
RUN cp target/debug/delivery-service target/release/delivery-service /dist 2>/dev/null | true;
