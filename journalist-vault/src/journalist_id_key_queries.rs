...
use common::{
    ...
    protocol::keys::{
        ...
        UnregisteredJournalistIdKeyPair, UntrustedJournalistIdKeyPair,
...
pub(crate) async fn insert_candidate_journalist_id_key_pair(
...
pub(crate) async fn candidate_journalist_id_key_pair(
    conn: &mut SqliteConnection,
...
pub(crate) async fn published_journalist_id_key_pairs(
    conn: &mut SqliteConnection,
    now: DateTime<Utc>,
    ...
    .flat_map(move |row| {
        ...
        let provisioning_pk = org_pks_from_trust_anchors
            ...
            .context(format!(
                "Could not verify provisioning pk for id key id {}",
                row.id
            ))?;
...
pub(crate) async fn insert_registered_journalist_id_key_pair(
    conn: &mut SqliteConnection,
...
pub(crate) async fn delete_candidate_journalist_id_key_pair(
    conn: &mut SqliteConnection,
    id_key_pair: &UnregisteredJournalistIdKeyPair,
) -> anyhow::Result<Option<CandidateJournalistIdKeyPairRow>> {
...
pub(crate) async fn last_published_journalist_id_key_pair_at(
    conn: &mut SqliteConnection,
...
pub(crate) async fn delete_expired_journalist_id_key_pairs(
    conn: &mut SqliteConnection,
    now: DateTime<Utc>,
) -> anyhow::Result<()> {
    sqlx::query!(
